...
class CROHME_OCR_Dataset(Dataset):
    ...
    def __getitem__(self, index):
        try:
            ...
            if pixel_values is None or pixel_values == [] or labels == [] or labels is None or len(labels)>self.max_target_length:
                return self[np.random.randint(len(self))]
        ...
        def __call__(self, features, return_tensors=None):
            try:
                if return_tensors is not None:
                    return self.inner(features, return_tensors)
                return self.inner(features)
            except:
                return self(features[1:],return_tensors)
